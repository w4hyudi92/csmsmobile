import { Component } from '@angular/core';
import { Platform, IonicPage, NavController, NavParams, AlertController, LoadingController } from 'ionic-angular';
import * as moment from 'moment';
import { Storage } from '@ionic/storage';
import { ApiProvider } from '../../providers/api/api';
import { DetailwoPage } from './../detailwo/detailwo';
import pdfMake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
import { FileOpener } from '@ionic-native/file-opener';
import { File } from '@ionic-native/file';
import { FileTransfer, FileTransferObject } from '@ionic-native/file-transfer';
import { ListwoPage } from '../listwo/listwo';
/**
 * Generated class for the FormwoPage page.
 *
 * See https://ionicframework.com/docs/components/#navigation for more info on
 * Ionic pages and navigation.
 */

@IonicPage()
@Component({
  selector: 'page-formwo',
  templateUrl: 'formwo.html',
  providers: [ApiProvider]
})
export class FormwoPage {
  maxTime: any=30;
  timer:any;
  hidevalue: any;
  hidekunjungan: any;
  start:any;
  end:any;

  timeInSeconds: any;
  time:any;
  runTimer:any;
  hasStarted:any;
  hasFinished:any;
  remainingTime:any;
  displayTime:any;
  myDate: any;

  public params;
  public waktu_kunjungan = [];
  ron: string;
  status: string;
  pdfObj = null;
  history:boolean = false;

  constructor(public navCtrl: NavController, public navParams: NavParams,
    private storage: Storage,
    public api: ApiProvider,
    private fileOpener: FileOpener,
    private plt: Platform,
    private file: File, 
    public atrCtrl: AlertController,
    private transfer: FileTransfer,
    public loadingCtrl: LoadingController,
    ) {

      this.params = navParams.get("item");
      console.log(this.params);
      this.status = this.params['status'];
      this.myDate = new Date();
  }

  ionViewDidLoad() {
    //this.storage.remove('waktu_kunjungan');
  }

  ionViewDidEnter(){
    this.storage.get('waktu_kunjungan').then(waktu => {
      //console.log(waktu);
      if(waktu != null){
        this.waktu_kunjungan=waktu;
        this.hidekunjungan = false;
        for(var k = 0; k < waktu.length; k++){
          if(waktu[k]['ron'] == this.params['nomor_ron']){
             this.hidekunjungan = true;
             this.start = waktu[k]['start_kunjungan'];
             this.end = waktu[k]['finish_kunjungan'];
             this.hasStarted = true;
             this.runTimer = true;
             
             if(waktu[k]['finish_kunjungan'] !=''){
              this.runTimer = false;
              this.hidevalue=false;
              var ms = moment(waktu[k]['finish_kunjungan'],"DD/MM/YYYY HH:mm:ss").diff(moment(waktu[k]['start_kunjungan'],"DD/MM/YYYY HH:mm:ss"));
              var d = moment.duration(ms);

              this.displayTime =  (d.hours()<10 ? '0' + d.hours() : d.hours()) + ':' + (d.minutes()<10 ? '0' + d.minutes() : d.minutes()) + ':' + (d.seconds()<10 ? '0' + d.seconds() : d.seconds());

             }

             this.timerTick();
          }
        }

        if(this.status != 'Terima' && this.status != 'Pending' ){
          this.hidekunjungan = true;
          this.runTimer = false;
          this.hidevalue = false;
          this.history = true;
        }
        
      }else{
        this.hidekunjungan = false;
      }
    });

    
  }

  startkunjungan(){
    this.initTimer();
    this.startTimer();
  }


  initTimer() {
    
    let arr_time = [];
    if (!this.timeInSeconds) { 
      this.timeInSeconds = 1500; 
    }
  
    this.time = this.timeInSeconds;
    this.runTimer = false;
    this.hasStarted = false;
    this.hasFinished = false;
    this.hidekunjungan = true;
    
    this.remainingTime = moment().format("DD/MM/YYYY HH:mm:ss");  
    this.start = this.remainingTime;
    this.waktu_kunjungan.push({
            start_kunjungan: this.remainingTime,
            ron: this.params['nomor_ron'],
            log_id: this.params['log_id'],
            finish_kunjungan: '',
            start_repair: '',
            finish_repair: ''
          });
    
    
    this.api.setmemory('waktu_kunjungan',this.waktu_kunjungan );

    this.displayTime = this.getSecondsAsDigitalClock();
   
  }

  startTimer() {
    this.runTimer = true;
    this.hasStarted = true;
    this.timerTick();
  }
  
  pauseTimer() {
    this.runTimer = false;
  }
  
  resumeTimer() {
    this.startTimer();
  }

  timerTick() {
    setTimeout(() => {

      if (!this.runTimer) { return; }

      this.displayTime = this.getSecondsAsDigitalClock();

      if (this.hasStarted) {
        this.hidevalue = false;
        this.timerTick();
      }else {
        this.runTimer = false;
        this.hidevalue = true;
      }
    }, 1000);
  }

  getSecondsAsDigitalClock() {
    var now = this.remainingTime;
    var then = moment().format("DD/MM/YYYY HH:mm:ss");  
    if(this.start){
      now= this.start;
    }
    var ms = moment(then,"DD/MM/YYYY HH:mm:ss").diff(moment(now,"DD/MM/YYYY HH:mm:ss"));
    var d = moment.duration(ms);

    return (d.hours()<10 ? '0' + d.hours() : d.hours()) + ':' + (d.minutes()<10 ? '0' + d.minutes() : d.minutes()) + ':' + (d.seconds()<10 ? '0' + d.seconds() : d.seconds());
  }

  goform(ronnumber) {
    //console.log(ronnumber);
    this.navCtrl.push(DetailwoPage,{
      ron: ronnumber,
      isi: this.params,
      waktu_kunjungan: this.waktu_kunjungan,
      history: this.history 
    });
    console.log(this.params['total']);
  }
  
  previewnota(){
    var ron = this.params['nomor_ron'];
    this.createPdf();
  }

  createPdf() {
    var nilai = this.params['total'];
    var diskon = nilai-(nilai*5/100);
    var total = 0;
    var diskonttp = 0;
    var keterangan = "";
    if(this.params['metodebayar'] == "E-Wallet"){
        if((nilai*5/100) > 50000){
            diskonttp = 50000;
            total = nilai-diskonttp;
            console.log(this.params['diskon_ron']);
        }else if(nilai*5/100 <= 50000){
            diskonttp = nilai*5/100;
            total = diskon;
            console.log(this.params['diskon_ron']);
        }
    }else{
        total = nilai;
    }
    
    if(this.params['keterangan_teknisi'] == "null"){
      keterangan = "-";
    }else{
      keterangan = this.params['keterangan_teknisi'];
    }

    this.storage.get('prof').then(profile => {
      console.log(this.params);
        var docDefinition = {
        footer: [
            {
            columns: [
            { text: ':: TERIMA KASIH TELAH MENJADI PARTNER MODENA ::', alignment: 'center',fontSize: 9 }
            ],
            },
            { text: ':: CALL CENTRE ::', alignment: 'center',fontSize: 9 },
            { text: '15.007.15', alignment: 'center',fontSize: 9 },
        ],
        content: [
            {
            columns: [
                {
                image: 'data:image/png;base64,',
                fit: [100, 100],
                style: 'foto'
                }]
            },
            { text: 'INVOICE REPAIR', style: 'header',marginTop:10 },
            { text:  this.params['tglinvoice'], fontSize: 9, alignment: 'center', marginBottom:5 },
            {
              canvas: [
              {   
                  type: 'line', 
                  x1: 0, 
                  y1: 5, 
                  x2: 220, 
                  y2: 5, 
                  lineWidth: 1 ,
              }]
            },
            { text: '', fontSize: 9, alignment: 'center', marginBottom:5 },
            {
              table: {
                  headerRows: 1,
                  style: 'tabel',
                  body: [
                    [ { text: 'No Nota', style: 'subheader' }, { text: ':', style: 'subheader' }, { text: (this.params.nomor_form=='null' || this.params.nomor_form=="undefined"? this.params.nomor_ron : (this.params.nomor_ron + '/' + this.params.nomor_form)) , style: 'subheader' } ],
                    [ { text: 'Teknisi',style: 'subheader'},{ text: ':', style: 'subheader' }, { text: profile.nama_lengkap, style: 'subheader' } ],
                    [ { text: 'Customer', style: 'subheader' },{ text: ':', style: 'subheader' }, { text: this.params['nama_konsumen'], style: 'subheader' } ],
                    [ { text: 'Keluhan', style: 'subheader'},{ text: ':', style: 'subheader' }, { text: this.params['model'] + ' / ' + this.params['keluhan'], style: 'subheader' } ],
                    [ { text: 'Pembayaran', style: 'subheader' },{ text: ':', style: 'subheader' }, { text: this.params['metodebayar'], style: 'subheader' } ],
                  ]
              }, 
              layout: 'noBorders'
            },
            {
              canvas: [
                {   
                    type: 'line', 
                    x1: 0, 
                    y1: 5, 
                    x2: 220, 
                    y2: 5, 
                    lineWidth: 1 ,
                }]
            },
            { text: '', fontSize: 9, alignment: 'center' },
            { text: 'Rincian Biaya Perbaikan :', style: 'story', margin: [0, 10, 0, 5]},
            {
              table: {
                  headerRows: 1,
                  widths: [ 100, 100],
                  body: [
                  [ { text: '1. Kunjungan', fontSize: 9 }, { text: (this.params['biaya_kunjungan']== null ? 0 : parseInt(this.params['biaya_kunjungan']).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")) , fontSize: 9,alignment: 'right', padding: 0,margin: 0 } ],
                  [ { text: '2. Material', fontSize: 9}, { text: (this.params['biaya_material']== null ? 0 : parseInt(this.params['biaya_material']).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")), fontSize: 9,alignment: 'right' } ],
                  [ { text: '3. Level Perbaikan', fontSize: 9 }, { text: parseInt(this.params['biaya_kerja']).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","), fontSize: 9,alignment: 'right' } ],
                  [ { text: '4. Lain-lain', fontSize: 9}, { text: (this.params['biaya_lainnya']== null ? 0 : parseInt(this.params['biaya_lainnya']).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")), fontSize: 9,alignment: 'right' } ],
                  [ { text: '5. Part', fontSize: 9, }, { text: (this.params['biaya_part']== null ? 0 : parseInt(this.params['biaya_part']).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")), fontSize: 9,alignment: 'right' } ],
                  //[ { text: 'Subtotal', fontSize: 9, bold: true}, { text: (this.params['total']== null ? 0 : parseInt(this.params['total']).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")), fontSize: 9,alignment: 'right' } ],
                  [ { text: 'Diskon ( - )', fontSize: 9, bold: true }, { text: (diskonttp== null ? 0 : diskonttp.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")), fontSize: 9,alignment: 'right' } ],
                  [ { text: '', fontSize: 9 }, { text: '', fontSize: 9 } ],
                  ]
              },
              layout: 'noBorders'
            },
            {
            canvas: [
            {   
                type: 'line', x1: 0, y1: 5, x2: 220, y2: 5, lineWidth: 1 ,
            }]
            },
            { text: '', fontSize: 9, alignment: 'center', marginBottom:5 },
            {
            columns: [
                { text: 'Total', fontSize: 10, bold: true, alignment: 'left' },
                { text: total.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","), fontSize: 10, bold: true, alignment: 'right',paddingRight: 5,marginRight: 10 }
            ],
            },
            {
            canvas: [
            {   
                type: 'line', x1: 0, y1: 5, x2: 220, y2: 5, lineWidth: 1 ,
            }]
            },
            { text: '', fontSize: 9, alignment: 'center', marginBottom:5 },
            { text: 'Note : ' + this.params['keterangan_teknisi'], fontSize: 9,italic: true, alignment: 'left' },
        ],
        pageSize: 'A6',
        styles: {
            
            foto: {
            alignment: 'center',
            },
            header: {
            fontSize: 11,
            bold: true,
            alignment: 'center',
            },
            subheader: {
            fontSize: 9,
            bold: true,
            },
            story: {
            fontSize: 9,
            italic: true,
            alignment: 'left',
            width: '50%',
            bold: true,
            }
        }
        }
        this.pdfObj = pdfMake.createPdf(docDefinition);
        this.downloadPdf(this.params['nomor_ron']);
    });
  }

  downloadPdf(ron) {
    //const loader = this.loadingCtrl.create({content: "Mohon Tunggu, Sedang menunggu respon server......"});
    //loader.present();

    this.pdfObj.getBuffer((buffer) => {
      var blob = new Blob([buffer], { type: 'application/pdf' });
      if (this.plt.is('cordova')) {
        this.file.writeFile(this.file.externalRootDirectory, 'Download/'+ ron +'.pdf', blob, { replace: true }).then(fileEntry => {
          const confirm = this.atrCtrl.create({
            title: 'Informasi',
            message: 'File dokumen '+ron+'.pdf berhasil di unduh',
            buttons: [
              {
                text: 'Tutup',
                handler: () => {
                  // console.log('Disagree clicked');
                }
              },
              {
                text: 'Buka',
                handler: () => {
                  this.fileOpener.open(this.file.externalRootDirectory + 'Download/' + ron + '.pdf', 'application/pdf')
                      .then(() => console.log('File is opened'))
                      .catch(e => console.log('Error opening file', e));
                }
              }
            ]
          });
          confirm.present();
          //loader.dismiss();
        });
      }else{
        this.pdfObj.download();
      }
    }, (error) => {
       console.log(error);
       const alert = this.atrCtrl.create({
         title: 'Informasi',
         subTitle: 'File dokumen gagal di unduh.',
         buttons: ['OK']
       });
       alert.present();
       //loader.dismiss();
     });
  }
}

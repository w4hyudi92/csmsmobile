...
@Component({
  ...
  templateUrl: 'modelinfoprts.html',
})
export class ModelinfoprtsPage {
  ...
  tambahkan(itm){
    ...
    if(Math.round(total+(itm['unitprice_disc']-(itm['unitprice_disc']*this.diskonjual/100)*this.qty))> this.navParams.get('limit') ){
    ...
    arrs['qty'] = this.qty;
    arrs['diskon'] = this.diskonjual;
    arrs['qty_avail'] = parseInt(itm['qty_avail']);
    arrs['unitprice'] = parseInt(itm['unitprice']);
    arrs['disc_currency'] = parseInt(itm['unitprice'])*this.diskonjual/100;
    arrs['unitprice_disc'] = Math.round(itm['unitprice_disc']-(itm['unitprice_disc']*this.diskonjual/100));

    this.navParams.get('content').total = Math.round(total + (itm['unitprice_disc']-(itm['unitprice_disc']*this.diskonjual/100)*this.qty));
  ...
  hitungtotal(){
    let tot:number = 0
    for(var k = 0; k < this.navParams.get('content').arrData.length; k++){
      tot = Math.round(tot + ( this.navParams.get('content').arrData[k]['unitprice_disc']-(this.navParams.get('content').arrData[k]['unitprice_disc']*this.diskonjual/100) * this.navParams.get('content').arrData[k]['qty']));
    }
    return tot;
  }
}

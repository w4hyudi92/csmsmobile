...
@IonicPage()
@Component({
  selector: 'page-successks',
  templateUrl: 'successks.html',
})
export class SuccessksPage {
    public idParam;
    header = [];
    headerItems = [];
    param: any;
    ...
    selesaiDong() {
		...
		this.navCtrl.setRoot(MenuksPage);
	}
    ...
    previewPdf() {
        let diskon = this.param['harga']*this.param['diskon']/100;
        let total = this.param['harga']-diskon;
        this.storage.get('prof').then(profile => {
            ...
            content: [
                ...
                { text: 'Tanggal Cetak : ' + this.tglcetak, fontSize: 10, marginBottom: 20, alignment: 'center'},
                {
                    table: {
                        ...
                        style: 'tabel',
                        body: [
                        [ { text: 'No Kontrak Service', style: 'subheader' }, { text: ':', style: 'subheader' }, { text: this.param['no_ks'] , style: 'subheader' } ],
                        [ { text: 'Teknisi',style: 'subheader'},{ text: ':', style: 'subheader' }, { text: profile.nama_lengkap, style: 'subheader' } ],
                        [ { text: 'No. Konsumen', style: 'subheader' },{ text: ':', style: 'subheader' }, { text: this.param['konsumen_card'], style: 'subheader' } ],
                        [ { text: 'Nama Konsumen', style: 'subheader' },{ text: ':', style: 'subheader' }, { text: this.param['nama_konsumen'], style: 'subheader' } ],
                        [ { text: 'Produk', style: 'subheader'},{ text: ':', style: 'subheader' }, { text: this.param['produk_id'] , style: 'subheader', marginBottom: 10 } ],
                ...
                table: {
                    ...
                    widths: [ 100, 100],
                    body: [
                    [ { text: '1. '+this.param['jnsks'], fontSize: 10 }, { text: parseInt(this.param['harga']).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") , fontSize: 10, bold: true, alignment: 'right' } ],
                ...
                columns: [
                    ...
                    { text: 'Rp. '+total.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","), fontSize: 10, bold: true, alignment: 'right',paddingRight: 5, marginRight: 10 }
                ]
                }
            ],
            pageSize: 'A6',
            ...
            this.downloadPdf(this.param['no_ks']);
        });
    }
